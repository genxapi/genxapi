...
jobs:
  generate:
    ...
    steps:
      ...
      - name: Install CLI
        run: npm install --no-save @eduardoac/generate-api-client

      - name: Generate clients
        env:
          GITHUB_TOKEN: ${{ secrets.GENERATOR_GITHUB_TOKEN }}
          NPM_TOKEN: ${{ secrets.NPM_AUTOMATION_TOKEN }}
        run: npx generate-api-client generate --log-level info
      ...
      - name: Publish release
        ...
        env:
          GITHUB_TOKEN: ${{ secrets.GENERATOR_GITHUB_TOKEN }}
        run: >
          npx generate-api-client publish
          --owner ${{ github.repository_owner }}
          --repo ${{ github.event.repository.name }}
          --tag v${{ github.run_number }}
          --title "Automated client refresh"
          --body "Generated by generate-api-client workflow."
```
    ...
    strategy:
      matrix:
        config:
          ...
          - services/payments/api-client-generatorrc.yaml
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-node@v4
        with:
          node-version: 20
      - run: npm install --no-save @eduardoac/generate-api-client
      - run: npx generate-api-client generate --config ${{ matrix.config }}
        env:
          GITHUB_TOKEN: ${{ secrets.GENERATOR_GITHUB_TOKEN }}
```
...
Prevent unapproved API changes from merging by running the upcoming `diff` command (preview):

```yaml
      - name: Diff OpenAPI
        run: >
          npx generate-api-client diff
          --base specs/petstore-main.yaml
          --head specs/petstore-pr.yaml
          --format summary
```

> 🚧 Note: The `diff` command is under active development. Use `packages/generate-api-client/src/utils/swaggerDiff` directly until the CLI command is released.

## Integrate with Other CI Providers

- **GitLab CI/CD** – run `npx generate-api-client generate` inside a Node.js image and set `GITHUB_TOKEN`/`NPM_TOKEN` as masked variables.
