...
jobs:
  generate:
    ...
    steps:
      ...
      - name: Install CLI
        run: npm install --no-save client-api-generator
      ...
      - name: Publish release
        ...
        run: >
          ...
          --body "Generated by client-api-generator workflow."
```
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 20
      - run: npm install --no-save client-api-generator
      - run: npx client-api-generator generate --config ${{ matrix.config }}
        env:
          GITHUB_TOKEN: ${{ secrets.GENERATOR_GITHUB_TOKEN }}
```
...
- **GitLab CI/CD** – run `npx client-api-generator generate` inside a Node.js image and set `GITHUB_TOKEN`/`NPM_TOKEN` as masked variables.
