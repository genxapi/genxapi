...
**What you see**
- Running the generator with the Kubb template (via `--template kubb` or a config that points at `@genxapi/template-kubb`) fails during `npm install` with:  
...
**How to fix it manually**
1. Open `packages/template-kubb/src/template/package.json`.
...
3. Run `npm run build --workspace @genxapi/template-kubb` so the compiled template under `dist/template/package.json` picks up the new versions.
...
**Permanent fix**
- The repository now pins the scaffolded Kubb dependencies to `^4.1.3`. Pull the latest changes or upgrade to the next published CLI build to get the updated `@genxapi/template-kubb` out-of-the-box.
